...
void Bank::Get_Data() {
  ...
  cout << "\nWhich type of Account \n[S for Saving Account] (or)[C for Current "
          "Account]: ";
  cin.clear();
  fflush(stdin);
  cin >> type;
...
void Bank::report() const {
  cout << sep << setw(maxNumber) << account_number << sep << setw(maxName)
       << holder_name << sep << setw(typeSize) << type << sep << setw(maxMoney)
       << deposit << sep << endl
       << sepLine << endl;
}

void Bank::dep(int x) { deposit = deposit + x; }

void Bank::draw(int x) { deposit -= x; }
    ...
    switch (choice) {
      ...
      case '8':
        cout << "\n\n\tThanks for using Bank Record System.\n\n\tFor more cool "
                "stuff, visit https//github.com/imabhisht";
...
void Get_All_Data() {
  ...
  cout << "\n\n\t\tACCOUNT HOLDER LIST\n\n";
  cout << sepLine << endl
       << sep << setw(maxNumber) << "A/C NO." << sep << setw(maxName) << "NAME"
       << sep << setw(typeSize) << "TYPE" << sep << setw(maxMoney) << "BALANCE"
       << sep << endl
       << sepLine << endl;
  ...
  while (!File.eof() && found == false) {
    ...
    if (call.retacno() == n) {
      ...
      if (option == 2) {
        ...
        if ((bal < 500 && call.rettype() == 'S') ||
            (bal < 1000 && call.rettype() == 'C'))
